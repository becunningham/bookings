...
func (m *postgresDBRepo) SearchAvailabilityByDatesByRoomID(start, end time.Time, roomID int) (bool, error) {
	...
	defer cancel()
	query := `
		select count(*) from room_restrictions where room_id= $1 and $2 < end_date and $3 > start_date
	`
	var numRows int
...
func (m *postgresDBRepo) GetUserByID(id int) (models.User, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
	query := `select id,first_name,last_name,email,password,access-level,created_at,updated_at
...
func (m *postgresDBRepo) AllNewReservations() ([]models.Reservation, error) {
	...
	query := `select r.id,r.first_name,r.last_name,r.email,r.phone,r.start_date,r.end_date,r.room_id,r.created_at,r.updated_at,r.processed,rm.id,rm.room_name
		from reservations r 
left join rooms rm on (r.room_id = rm.id) where r.processed=0
order by r.start_date asc
`
	rows, err := m.DB.QueryContext(ctx, query)
	defer rows.Close()
